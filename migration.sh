...
if $prompt_for_db_info ; then
    ...
    DB_HOST=$(gum input --placeholder "db.hmjirohmtymjjeqchqqx.supabase.co")
    DB_NAME=$(gum input --placeholder "postgres")
    DB_PORT=$(gum input --placeholder "5432")
    DB_USER=$(gum input --placeholder "postgres")
    DB_PASSWORD=$(gum input --placeholder "NH9cTtVHneXykC7o" --password)
